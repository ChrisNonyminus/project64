...
getDefaultPartial() {
    ...
    partial->Set(VSPEC::NAME, "Project64");
    ...
    partial->Set(VSPEC::SYSTEM, "Nintendo 64");
...
public
    ref class RDRAM : RTCV::CorruptCore::IMemoryDomain {
public:
...
#pragma region RDRAM
String ^ RDRAM::Name::get() {
    return "RDRAM";
}

long long RDRAM::Size::get() 
{
    int ramsize = ManagedWrapper::GetMemSize();
    return ramsize;
}

int RDRAM::WordSize::get() {
    return WORD_SIZE;
}

bool RDRAM::BigEndian::get() {
    return BIG_ENDIAN;
}

unsigned char RDRAM::PeekByte(long long addr) {
    //return ReadMem8(static_cast<u32>(addr));
    if (addr < RDRAM::Size)
    {
        return ManagedWrapper::peekbyte(addr + 0x0);
        //return ManagedWrapper::peekbyte(addr);
    }
    else return 0;
}

void RDRAM::PokeByte(long long addr, unsigned char val) {
    //WriteMem8(static_cast<u32>(addr), val);
    if (addr < RDRAM::Size)
    {
        ManagedWrapper::pokebyte(addr + 0x0, val);
        //ManagedWrapper::pokebyte(addr, val);
    }
    else return;
}

cli::array<unsigned char> ^ RDRAM::PeekBytes(long long address, int length) {
...
static cli::array<MemoryDomainProxy^>^ GetInterfaces() {
    ...
    interfaces[0] = (gcnew MemoryDomainProxy(gcnew RDRAM));
    return interfaces;
}
...
void VanguardClientUnmanaged::LOAD_GAME_DONE() {
    ...
    try {
        gameDone->Set(VSPEC::SYSTEM, "Nintendo 64");
        gameDone->Set(VSPEC::SYSTEMPREFIX, "Nintendo 64");
        gameDone->Set(VSPEC::SYSTEMCORE, "Project64");
